...
class CuptiSchedulingProfiler {
 private:
  ...
  std::vector<double> inference_kernel_durations_us_;  // per-kernel GPU exec time for non-benchmark kernels
  bool collect_inference_kernels_ = false;             // gated: only collect during measurement phase
  int rejected_latency_count_ = 0;                    // count of rejected latency measurements
 ...
 public:
  ...
  void cleanup() {
    ...
    if (rejected_latency_count_ > 0) {
      std::cout << "[CUPTI] Total rejected latency measurements: " << rejected_latency_count_
                << " (timestamp underflow, clamped to 0 where possible)" << std::endl;
    }
  ...
  // --- Inference kernel execution time tracking ---

  // Start collecting execution durations for non-benchmark (inference) kernels.
  // Call after warmup to avoid measuring TRT engine build kernels.
  void startCollectingInferenceKernels() {
    std::lock_guard<std::mutex> lock(data_mutex_);
    inference_kernel_durations_us_.clear();
    collect_inference_kernels_ = true;
  }

  // Stop collecting and return all collected per-kernel execution durations.
  std::vector<double> stopAndGetInferenceKernelDurations() {
    std::lock_guard<std::mutex> lock(data_mutex_);
    collect_inference_kernels_ = false;
    std::vector<double> result;
    result.swap(inference_kernel_durations_us_);
    return result;
  }
      ...
      if (record->kind == CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL) {
        ...
        if (is_timing_kernel) {
          ...
          if (it != launch_map_.end()) {
            ...
            // Use signed arithmetic: if the GPU picks up the kernel before the
            // CUPTI timestamp callback completes, start < launch_timestamp. Treat these as
            // near-zero latency instead of rejecting them.
            int64_t latency_ns = (int64_t)(kernelRecord->start) -
                                 (int64_t)(it->second.launch_timestamp);
            ...
            // Clamp slightly-negative latencies to 0 (GPU started before timestamp recorded)
            if (latency_us < 0.0 && latency_us > -100.0) {
              latency_us = 0.0;
            }

            // Sanity check - latency should be reasonable (0 to 10ms)
            if (latency_us >= 0.0 && latency_us <= 10000.0) {
              ...
              rejected_latency_count_++;
              if (rejected_latency_count_ <= 5) {
                std::cout << "[CUPTI] WARNING: Rejected latency " << latency_us
                          << " μs (expected 0-10000 μs)" << std::endl;
              }
            }
          ...
          // Non-benchmark kernel
          // Track its execution duration if collection is enabled
          if (collect_inference_kernels_) {
            double exec_ns = (double)(kernelRecord->end - kernelRecord->start);
            double exec_us = exec_ns / 1000.0;
            if (exec_us > 0.0) {
              inference_kernel_durations_us_.push_back(exec_us);
            }
          }

          // Clean up launch_map entry if present
          auto it = launch_map_.find(kernelRecord->correlationId);
          if (it != launch_map_.end()) {
            launch_map_.erase(it);
          }
